...
def conversionHexMac(adresse):
	adresseHex=str()
	for element in adresse.split(':'): adresseHex+=element
...
def listeAdresse(fourchette):
	...
	if len(fourchette.split('-')) == 2:
		...
		borneMin=tmp[3]
		if int(borneMin) > int(borneMax): liste.append(False)
		else:
			for i in range(int(borneMin), int(borneMax)+1): liste.append(statique+str(i))
		
	elif len(fourchette.split('-')) == 1: liste.append(fourchette)
			...
			try:
				...
				if len(rep) == 64:
					...
					if proto == "0806" and op == '2' and ip == conversionHexIP(cible): 
						print("> [{}] {} ({})".format(socket.gethostbyaddr(cible)[0], cible, conversionGraphMac(mac)))
						
			except socket.timeout: pass
			except socket.gaierror: parser.print_help()
		
		s.close()
	else: parser.print_help()

if __name__ == '__main__':
	parser=argparse.ArgumentParser(prog='hlist4', description='Découverte d\'hôte via ARP')
	parser.add_argument('-i', action='store', metavar='interface', required=True, help='Interface à utiliser')
	parser.add_argument('-f', action='store', metavar='fourchette', required=True, help='Adresse(s) à scanner (ex: 192.168.1.21-52 ou 192.168.1.21)')
	...
	if maMac == False or monIp == False: print("> [ERR] interface %s non trouvée"%interface) 
	else: envoiRequete(interface, liste, maMac, monIp)
